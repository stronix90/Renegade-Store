...
const Checkout = () => {
  ...
  const [purchase, setPurchase] = useState({items:[], cantidadTotal: 0, montoTotal: 0});

  const purchaseItemsWithoutImage = () => purchase.items.map((p) => ({
    id: p.id,
    title: p.title,
    price: p.price,
    q: p.q,
  ...
  useEffect(() => {setPurchase({items:carrito, cantidadTotal: cantidadTotal, montoTotal: montoTotal})}, []);
  

  return (
    <div className="container customContainer" style={{ paddingTop: "12rem" }}>
      <div className="row justify-content-around">
        ...
        <div className="card itemCard neutral800 col-lg-6 my-2 my-lg-0">
          <div className="card-body">
            ...
            <div className="row checkout_steps">
              <div className={purchaseId ? "col-auto done": "col-auto undone"}>
                <span>2</span>
              </div>
            ...
            {(user && !purchaseId) && (
              <div className="row">
                <button className="customBtn d-block" onClick={savePurchase}>
                  CONFIRMAR
                </button>
              </div>
            )}
            ...
            {purchaseId && (
              <div className="row">
                <p className="text-center">Conserve su comprobante: {purchaseId}</p>
              </div>
            )}
          </div>
        </div>
